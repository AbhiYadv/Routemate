...
export default function Bookings() {
  ...
  const TabButton = ({ title }: { title: any }) => (
    <TouchableOpacity 
      style={tw`px-4 py-2 ${activeTab === title ? 'border-b-2 border-[#1d4ed8]' : ''}`}
      onPress={() => setActiveTab(title)}
    >
      <Text style={tw`${activeTab === title ? 'text-[#1d4ed8] font-bold' : 'text-gray-500 font-medium'}`}>{title}</Text>
    </TouchableOpacity>
  );
            ...
            <View key={idx} style={tw`bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-4`}>
              ...
              <View style={tw`bg-[#F8FAFC] rounded-xl p-4 border border-gray-100 mb-4`}>
                <View style={tw`flex-row items-center mb-2`}>
                  <View style={tw`w-2 h-2 rounded-full bg-[#1d4ed8] mr-3`} />
