    ...
    <SafeAreaView style={tw`flex-1 bg-[#F0F4FF] pt-${Platform.OS === 'android' ? '8' : '0'}`}>
      ...
      */}
        ...
        <View style={tw`bg-white mx-4 -mt-5 rounded-2xl p-5 shadow-lg border border-gray-100`}>
          ...
          <View style={tw`flex-row flex-wrap gap-2`}>
            <View style={tw`bg-green-50 px-3 py-1.5 rounded-full flex-row items-center border border-green-100`}>
              <Ionicons name="shield-checkmark" size={13} color="#16a34a" style={tw`mr-1`} />
        ...
        <View style={tw`mx-4 mt-3 bg-white rounded-2xl border border-gray-100 shadow-sm`}>
          <View style={tw`flex-row`}>
            {[
              ...
              { label: 'Seats Left', value: `${ride.available_seats}/${ride.total_seats}`, color: '#16a34a' },
            ].map((stat, i) => (
            ...
            {ride.stop_sequence?.map((stop: any, idx: number) => (
              <View key={idx} style={tw`flex-row mb-4 relative`}>
                ...
                <View style={[
                  ...
                  { backgroundColor: stop.type === 'PICKUP' ? '#16a34a' : '#ea580c' },
                ]}>
      ...
      <View style={tw`bg-white px-5 py-4 border-t border-gray-100 shadow-[0_-2px_8px_rgba(0,0,0,0.06)]`}>
        ...
        <TouchableOpacity
          style={tw`w-full bg-[#16a34a] py-4 rounded-xl items-center shadow-md ${ride.available_seats === 0 || booking ? 'opacity-50' : ''}`}
          onPress={handleBook}
      ...
      <Modal visible={showCallModal} transparent animationType="fade">
        <View style={tw`flex-1 bg-[#0F172A]/95 justify-center items-center p-6`}>
          ...
          <View style={tw`w-28 h-28 bg-[#14532d] rounded-full mb-5 items-center justify-center border-2 border-[#16a34a]/40`}>
